...
ALTER TABLE `oauth_user`
    ADD `refresh_token` VARCHAR(255) NULL  DEFAULT NULL  AFTER `user_id`;
ALTER TABLE `oauth_user`
    ADD `union_id` VARCHAR(255) NULL  DEFAULT NULL  AFTER `refresh_token`;
ALTER TABLE `oauth_user`
    ADD `expires_in` VARCHAR(255) NULL  DEFAULT NULL  AFTER `union_id`;

INSERT INTO `system_config` (`id`, `key`, `value`, `description`, `pid`, `type`, `option`, `reboot`)
VALUES (54, NULL, NULL, 'WeChat配置', 0, '', NULL, 0),
